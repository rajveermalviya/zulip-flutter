...
class LiveGlobalStore extends GlobalStore {
  ...
  ///
  /// When executing on the main Runner Xcode target, it may copy the database
  /// file from its filesystem to the iOS App Group container, if not already
  /// copied.
  ///
  /// This copying is required for other Xcode targets (e.g.
  ...
  /// target's filesystem and thus it can't copy the file.
  ///
  ...
  static Future<File> _iosDbFile() async {
    ...
    if (IosNotificationService.isExecutingInExtension) {
      ...
      throw Exception('Database file found not found in the iOS App Group '
        'container.');
    }

    // Copy the database file from Runner target's filesystem which was stored
